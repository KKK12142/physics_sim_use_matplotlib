...
class Box(PhysicsObject):
    ...
    def get_surface_transform(self, t=0.5):
        """박스 윗면 위 t 위치의 로컬 좌표계 반환

        Args:
            t: 0=왼쪽 끝, 1=오른쪽 끝

        Returns:
            Transform2D: 박스 윗면 위 위치
        """
        # 윗면의 양 끝점
        top_left = self._corners[3]
        top_right = self._corners[2]

        # t에 따른 위치 보간
        surface_x = top_left.x + t * (top_right.x - top_left.x)
        surface_y = top_left.y + t * (top_right.y - top_left.y)

        return Transform2D(
            origin=(surface_x, surface_y),
            angle_rad=np.radians(self._rotation)
        )
...
class Ground(PhysicsObject):
    ...
    def get_t_for_x(self, x):
        """x 좌표에 해당하는 t 값 반환 (역함수)
...
class Ceiling(PhysicsObject):
    ...
    def get_surface_transform(self, t=0.5):
        ...
        Returns:
            Transform2D: 천장 아래 위치 (수평, 각도 0)
        """
        # t에 따른 x 위치
        x = self._x_min + t * self._width
        origin = Point(x, self._bottom_y)

        # 천장은 수평 (각도 0)
        return Transform2D(origin, angle_rad=0)
...
class Wall(PhysicsObject):
    ...
    Args:
        x: 벽의 x좌표
        ylim: (y_min, y_max) 벽 범위
        depth: 빗금 깊이
        side: 'left' (왼쪽 벽, 빗금 왼쪽) 또는 'right' (오른쪽 벽, 빗금 오른쪽)
    """

    def __init__(self, ax, x=0, ylim=(0, 3), depth=0.3, side='left'):
        super().__init__(ax)
        ...
        if side == 'left':
            self._center = Point(x - depth / 2, (y_min + y_max) / 2)
            self._surface_x = x  # 오른쪽 면이 표면
        else:
            self._center = Point(x + depth / 2, (y_min + y_max) / 2)
            self._surface_x = x  # 왼쪽 면이 표면

        self._width = depth
        self._height = y_max - y_min

        # 벽 선
        ax.plot([x, x], [y_min, y_max], 'k-', lw=3)

        # 빗금
        if side == 'left':
            for i in np.arange(y_min, y_max, 0.3):
                ax.plot([x - depth, x], [i + 0.3, i], 'k-', lw=1)
        else:
            for i in np.arange(y_min, y_max, 0.3):
                ax.plot([x, x + depth], [i, i + 0.3], 'k-', lw=1)
    ...
    def get_surface_transform(self, t=0.5):
        ...
        Returns:
            Transform2D: 벽 표면 위치 (수직, 각도 90도 또는 -90도)
        """
        # t에 따른 y 위치
        y = self._y_min + t * self._height
        origin = Point(self._surface_x, y)

        # 벽은 수직 (왼쪽 벽: 오른쪽으로 법선, 오른쪽 벽: 왼쪽으로 법선)
        if self._side == 'left':
            angle = 0  # 오른쪽 방향이 접선
        else:
            angle = np.pi  # 왼쪽 방향이 접선

        return Transform2D(origin, angle_rad=angle)

    def point_at(self, t=0.5):
        """벽 표면의 특정 높이 점 반환"""
        y = self._y_min + t * self._height
        return Point(self._surface_x, y)
    ...
    def __init__(self, ax, x=0, y=0, radius=None, on=None, t=0.5,
        ...
        if on is not None:
            ...
            if hasattr(on, 'get_surface_transform'):
                ...
                # Ground인 경우 특별 처리 (바닥 위에 설치)
                if isinstance(on, Ground):
                    if radius is None:
                        radius = 0.25
                    self._radius = radius
                    self._width = radius * 2
                    ...
                    # Ground의 t 위치에서 반지름만큼 위로 (+ offset)
                    ground_x = on._center.x - on._width / 2 + t * on._width
                    self._center = Point(ground_x, on._top_y + radius*2 + offset)

                    # 지지대 그리기 (바닥에서 도르래 아래쪽까지)
                    if show_support:
                        support_base = Point(ground_x, on._top_y)
                        support_top = Point(ground_x, self._center.y - radius)
                        self._draw_support(ax, support_base, support_top,
                                           support_width, edgecolor, zorder - 1)

                # Ceiling인 경우 특별 처리
                elif isinstance(on, Ceiling):
                    if radius is None:
                        radius = 0.25
                    self._radius = radius
                    self._width = radius * 2
                    ...
                    # 천장에서 반지름만큼 아래로 (+ offset)
                    ceiling_x = on._x_min + t * on._width
                    self._center = Point(ceiling_x, on._bottom_y - radius - offset)

                    # 지지대 그리기 (천장에서 도르래 위쪽까지)
                    if show_support:
                        support_base = Point(ceiling_x, on._bottom_y)
                        support_top = Point(ceiling_x, self._center.y + radius)
                        self._draw_support(ax, support_base, support_top,
                                           support_width, edgecolor, zorder - 1)
                else:
                    # Incline, Table 등 기존 처리
                    self._surface_transform = transform
...
class Spring:
    ...
    Args:
        obj1: 왼쪽 물체 (right 속성 사용)
        obj2: 오른쪽 물체 (left 속성 사용)
        start, end: 직접 좌표 지정 시 사용 (obj1, obj2 대신)
        n_coils: 코일 수 (기본 6)
        amplitude: 진폭 (기본 0.15)
        color: 선 색상
        lw: 선 두께
    """

    def __init__(self, ax, obj1=None, obj2=None, start=None, end=None,
                 n_coils=6, amplitude=0.15, color='black', lw=1.5):

        # 시작점/끝점 결정
        if obj1 is not None and obj2 is not None:
            # 물체의 right/left 사용
            if hasattr(obj1, 'right'):
                start = obj1.right
            elif hasattr(obj1, 'center'):
                start = obj1.center
            else:
                start = Point(obj1)

            if hasattr(obj2, 'left'):
                end = obj2.left
            elif hasattr(obj2, 'center'):
                end = obj2.center
            else:
                end = Point(obj2)
        else:
        ...
        mask = (t > flat_ratio) & (t < 1 - flat_ratio)
        t_wave = (t[mask] - flat_ratio) / (1 - 2 * flat_ratio)
        wave[mask] = np.sin(2 * np.pi * n_coils * t_wave) * amplitude

        # 곡선 좌표
        xs = start.x + t * dx + wave * px
        ys = start.y + t * dy + wave * py

        # 그리기
        ax.plot(xs, ys, color=color, lw=lw)

        # 양 끝 수직선 (용수철 끝 표시)
        end_bar_length = amplitude * 0.8
        # 시작점 수직선
        ax.plot([start.x - end_bar_length * px, start.x + end_bar_length * px],
                [start.y - end_bar_length * py, start.y + end_bar_length * py],
                color=color, lw=lw)
        # 끝점 수직선
        ax.plot([end.x - end_bar_length * px, end.x + end_bar_length * px],
                [end.y - end_bar_length * py, end.y + end_bar_length * py],
                color=color, lw=lw)
